import torch
import triton
from softmax_torch import naive_softmax
...
def benchmark(M, N, provider):
    ...
    if provider == 'triton':
        # ms, min_ms, max_ms = triton.testing.do_bench(lambda: softmax_triton(x, torch.empty([M, N], device=x.device, dtype=torch.float32), x.stride(0), x.stride(0)), quantiles=quantiles)
